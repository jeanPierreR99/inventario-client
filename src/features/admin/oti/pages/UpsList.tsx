...
const UPSList = () => {
    ...
    const fetchUps = async (currentPage: number) => {
        if (loading) return
        setLoading(true)
        try {
            const query = buildQuery()
            if (query) {
                const data = await API.getAllDataFilterOtiUps(query)
                setUpsList(data)
                setHasMore(false)
            } else {
                const response = await API.getAllDataOtiUps(currentPage, LIMIT)
                const data = response.data || response
                setUpsList(prev => (currentPage === 1 ? data : [...prev, ...data]))
                    ...
                    <tbody>
                        {upsList.map(item => (
                            <tr key={item.id} className="border-t hover:bg-gray-50">
                                ...
                                <td className="px-4 py-2">{item.power_capacity || "-"}</td>
                                <td className="px-4 py-2">{item.battery_runtime || "-"}</td>
