...
const RouterSearch = () => {
    ...
    const fetchRouter = async () => {
        try {
            if (!term) return;
            setRouters([]);
            const response = await API.getByCodeOtiRouter(term);
            setRouters(response);
        } catch (error) {
            console.error(error);
        }
    ...
    if (!routers || routers.length === 0) {
        return (
            <div>
                <PageTitle title="Buscar routers" />
                <div className="flex gap-2">
                    ...
                    />
                    <Button onClick={fetchRouter}>Buscar</Button>
                </div>
    ...
    const nodes = [
        {
            ...
            data: {
                label: buildNodeLabel(
                    '📡',
                    `${router.denomination || 'Router'} (${router.model || ''})`,
                    'router',
                    router,
                    `Serie: ${router.serial_number || 'N/A'}`,
                    `IP: ${router.ip || 'N/A'}`
                )
            },
            position: { x: 400, y: 0 },
        ...
        <div className="w-full h-[80vh]">
            ...
            <div className="flex flex-wrap gap-4 items-center text-sm mt-4">
                <div className="flex items-center gap-1">
                    <span className="w-4 h-4 rounded" style={{ background: '#f59e0b' }}></span> Router
                </div>
                <div className="flex items-center gap-1">
