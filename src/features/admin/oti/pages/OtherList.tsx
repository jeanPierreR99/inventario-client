...
const OtherList = () => {
    ...
    const fetchOthers = async (currentPage: number) => {
        if (loading) return
        setLoading(true)
        try {
            const query = buildQuery()
            if (query) {
                const data = await API.getAllDataFilterOtiOther(query)
                setOthers(data)
                setHasMore(false)
            } else {
                const data = await API.getAllDataOtiOther(currentPage, LIMIT)
                setOthers(prev => (currentPage === 1 ? data : [...prev, ...data]))
    ...
    useEffect(() => { fetchOthers(page) }, [page])
    ...
    return (
        <div>
            <PageTitle title="Lista de otros equipos" />
                            ...
                            <tr key={item.id} className="border-t hover:bg-gray-50">
                                ...
                                <td className="px-4 py-2">
                                    <Button size="sm" variant="outline" className="p-0">
                                        <NavLink target="_blank" className="p-2" to={`/oti/search-other/${item.patrimonial_code}`}>
                                            <Eye />
                                        </NavLink>
                                    </Button>
                                </td>
