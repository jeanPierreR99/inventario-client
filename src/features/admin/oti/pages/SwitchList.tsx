...
const SwitchList = () => {
    ...
    const fetchSwitches = async (currentPage: number) => {
        if (loading) return
        setLoading(true)
        try {
            const query = buildQuery()
            if (query) {
                const data = await API.getAllDataFilterOtiSwitch(query)
                setSwitches(data)
                setHasMore(false)
            } else {
                const response = await API.getAllDataOtiSwitch(currentPage, LIMIT)
                const data = response.data || response
                setSwitches(prev => (currentPage === 1 ? data : [...prev, ...data]))
    ...
    useEffect(() => { fetchSwitches(page) }, [page])
                ...
                <table className="min-w-full text-xs">
                    <thead className="bg-gray-100 text-sm">
                        <tr>
                            ...
                            <th className="px-4 py-2 text-left">Puertos</th>
