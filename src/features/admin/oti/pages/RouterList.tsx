...
const RouterList = () => {
    const [routers, setRouters] = useState<IOtiRouter[]>([])
    ...
    const fetchRouters = async (currentPage: number) => {
        if (loading) return
        setLoading(true)
        try {
            const query = buildQuery()
            if (query) {
                const data = await API.getAllDataFilterOtiRouter(query)
                setRouters(data)
                setHasMore(false)
            } else {
                const response = await API.getAllDataOtiRouter(currentPage, LIMIT)
                const data = response.data || response
                setRouters(prev => (currentPage === 1 ? data : [...prev, ...data]))
    ...
    useEffect(() => { fetchRouters(page) }, [page])
    ...
    return (
        <div>
            <PageTitle title="Lista de routers" />
            ...
            <div className="overflow-x-auto bg-white">
                <table className="min-w-full text-xs">
                    ...
                    <tbody>
                            ...
                            <tr key={item.id} className="border-t hover:bg-gray-50">
                                ...
                                <td className="px-4 py-2">
                                    <Button size="sm" variant="outline" className="p-0">
                                        <NavLink target="_blank" className="p-2" to={`/oti/search-router/${item.patrimonial_code}`}>
                                            <Eye />
                                        </NavLink>
                                    </Button>
                                </td>
                        ...
                        {filteredRouters.length === 0 && !loading && (
                            <tr>
