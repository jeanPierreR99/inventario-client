...
import ModalIpAvaialble from "../components/ModalIpAvaialble";
...
const ComputerList = () => {
  ...
  const [selectedGeneralOffice, setSelectedGeneralOffice] =
    useState<string>("");
  ...
  const fetchComputers = async (currentPage: number) => {
    ...
    try {
      ...
      if (query) {
        ...
        if (segment.length > 0) {
          const response = await API.apiAvailable(
            segment[0],
            segment[1],
            segment[2]
          );
  ...
  const handleOfficeChange = (value: string) => {
    ...
    setSelectedUnit("");
    const sede = sedeData.find((s) => s.name === selectedSede);
    const general = sede?.generalOffices?.find(
      (g) => g.name === selectedGeneralOffice
    );
    const office: any = general?.offices?.find((o) => o.name === value);
    const aux = office?.segments || [];
  ...
  return (
    <div>
      ...
      <div className="flex gap-4">
        ...
        <ModalIpAvaialble ips={selectedData} />
      </div>
            ...
            {filteredComputers.map((pc, index) => (
              <tr key={pc.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-2">
                  {pc.create_at
                    ? new Date(pc.create_at).toLocaleDateString()
                    : "-"}
                </td>
                <td className="px-4 py-2">
                  <div className="flex flex-col gap-1">
                    <div>
                      <strong>Sede:</strong> {pc.sede || "-"}
                    </div>
                    <div>
                      <strong>Oficina general:</strong>{" "}
                      {pc.generalOffice || "-"}
                    </div>
                    <div>
                      <strong>Oficina:</strong> {pc.office || "-"}
                    </div>
                    <div>
                      <strong>Unidad:</strong> {pc.unit || "-"}
                    </div>
                  </div>
                </td>
                ...
                <td className="px-4 py-2">
                  <Button size="sm" variant="outline" className="p-0">
                    <NavLink
                      target="_blank"
                      className="p-2"
                      to={`/oti/search-computer/${pc.patrimonial_code}`}
                    >
                      <Eye />
                    </NavLink>
                  </Button>
